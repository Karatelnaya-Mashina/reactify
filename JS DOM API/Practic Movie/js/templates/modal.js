export default class ModalWindow extends HTMLElement {
	...
	connectedCallback() {
		this.addEventListener('click', e => {
			...
			const categoryModal = this.modalCategory;
			...
			} else if (e.target.matches('#modal-btn-category > span')) {
			...
			if (e.target.matches('.modal__btn-category > svg')) {
			...
			document
				.querySelector('#modal-category .modal__box-category')
				.addEventListener('click', e => [(e._isClickWithInModal = true)]);
	...
	get btnOpenModalResponseCategory() {
		return this.querySelector('#modal-btn-category');
	}
	get modalCategory() {
		return this.querySelector('.modal__category');
	}
}
